...
def update_contribution_rate(
    ...
    db: Session = Depends(get_db), current_user=Depends(get_current_org_admin),
):
    jurisdiction = service.get_jurisdiction_by_id(db, current_user.organization_id, jurisdiction_id)
...
def activate(db: Session = Depends(get_db), current_user=Depends(get_current_org_admin)):
...
def deactivate(db: Session = Depends(get_db), current_user=Depends(get_current_org_admin)):
