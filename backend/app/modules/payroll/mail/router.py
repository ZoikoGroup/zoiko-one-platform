...
def ignore(message_id: int, db: Session = Depends(get_db), current_user=Depends(get_current_org_admin)):
...
def poll_now(db: Session = Depends(get_db), current_user=Depends(get_current_org_admin)):
