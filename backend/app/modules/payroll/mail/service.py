...
----------------------------------
Business logic for Payroll email settings (SMTP send identity only).

Tenant isolation follows the exact convention already used everywhere else
in this module (_apply_org_filter). Nothing here touches SMTP credentials
except to read whatever an org admin has already entered through the
settings endpoint this same submodule exposes.
"""
