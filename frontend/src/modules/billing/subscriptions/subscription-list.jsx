...
export default function SubscriptionsPage() {
  ...
  const handleRefresh = () => { setRefreshing(true); fetchSubscriptions(); };
  ...
  const StatusBadge = ({ status }) => {
    ...
    return (
      <span className={`inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-medium ${styles[status] || "bg-gray-100 text-gray-700"}`}>
        {status === "active" ? <CheckCircle size={12} /> : status === "paused" ? <PauseCircle size={12} /> : status === "cancelled" ? <XCircle size={12} /> : <Clock size={12} />}
        {status || "unknown"}
      </span>
    );
  };
  ...
  if (loading) {
    return (
      <HRPage title="Subscriptions" subtitle="Manage subscriptions">
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-slate-50 border-b border-slate-100">
                ...
                <th className="px-4 py-3 text-left text-xs font-semibold text-slate-500 uppercase tracking-wider">Next Billing</th>
              ...
              {subscriptions.length === 0 ? (
                <tr>
                  <td colSpan={7} className="px-4 py-16 text-center">
                    <div className="flex flex-col items-center">
                      ...
                      <p className="text-slate-400 text-sm mt-1">{search || statusFilter ? "Try adjusting your search or filters" : "No subscriptions yet"}</p>
                    </div>
                  </td>
                </tr>
                ...
                <tr key={sub.id} className="hover:bg-slate-50 transition-colors">
                  <td className="px-4 py-4">
                    <button onClick={() => navigate(`/billing/subscriptions/${sub.id}`)} className="font-medium text-slate-800 hover:text-violet-600 transition-colors">
